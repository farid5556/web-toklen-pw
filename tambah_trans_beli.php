...
<body>
    <div class="container">
        ...
        <main>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo '<td>';
                            echo '<button class="edit" onclick="window.location.href=\'edit_trans_beli.php?id=' . $row['no_faktur'] . '\'">Edit</button> ';
            ...
            <form action="submit_trans_beli.php" method="post">
                ...
                <div class="form-group">
                    ...
                    <select id="kode_suplier" name="kode_suplier" required>
                        <?php
                        include 'config.php';
                        $sql = "SELECT kode_suplier, nama_suplier FROM suplier";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                echo "<option value='" . $row['kode_suplier'] . "'>" . $row['kode_suplier'] . "</option>";
                            }
                        } else {
                            echo "<option value=''>Tidak ada suplier</option>";
                        }
                        $conn->close();
                        ?>
                    </select>
                ...
                <button type="submit" style="background-color: #430404;">Tambah Pembayaran</button>
            </form>
        </main>
    </div>
    <script>
        function confirmDelete(id) {
            if (confirm('Apakah Anda yakin ingin menghapus pembelian ini?')) {
                window.location.href = 'hapus_trans_beli.php?id=' + id;
            }
        }
    </script>
</body>
